...
#define SIZE_COLOR 32 //size of character array to store color
...
/*
-fix /tele
-make it say which player voted for which captain

-stop captains from sending out until they are the last player remaining on team (optional in config)

-swap captain to different teammate
-hook player spawn for captain actions
-rename and color players according to teams

-Sader idea, recall players for -10 hp loss
...
new String:g_Client_Color[MAXPLAYERS+1][SIZE_COLOR];
new String:g_Client_Name[MAXPLAYERS+1][MAX_NAME_LENGTH];
//======================================//
//										//
//			Game Globals				//
//										//
...
public OnPluginStart()
{
	...
	RegServerCmd("send_bchess_state",  CMD_SendState, "called by teleport menu to get participation state of a client");
	RegConsoleCmd("chess_start", CMD_ChatStart, "starts bchess via chat trigger");
}

public Action:Event_NameChange(Handle:event, const String:name[], bool:dontBroadcast)
{
	//stop name change message
	return Plugin_Handled;
}

public Action:CMD_SendState(args)
{
	new String:sz_target[4];
	GetCmdArg(1, sz_target, sizeof(sz_target));
	
	new client = StringToInt(sz_target);
	
	ServerCommand("get_bchess_state %d %d", client, g_Client_Data[client][PARTICIPATION_VALUE]);
...
public PlayerMissedSendOutTeam1(client)
{
	if( IsValidClient(client) )
	{
		if( !IsParticipant(client) )
		{
			SDKUnhook(client, SDKHook_SpawnPost, PlayerMissedSendOutTeam1);
		}
		else if( IsParticipant(client) && IsPlayerAlive(client) )
		{
			TeleportEntity(client, g_vec_Team1_send_pos, NULL_VECTOR, NULL_VECTOR);
			SDKUnhook(client, SDKHook_SpawnPost, PlayerMissedSendOutTeam1);
		}
	}
}

public PlayerMissedSendOutTeam2(client)
{
	if( IsValidClient(client) )
	{
		if( !IsParticipant(client) )
		{
			SDKUnhook(client, SDKHook_SpawnPost, PlayerMissedSendOutTeam2);
		}
		else if( IsParticipant(client) && IsPlayerAlive(client) )
		{
			TeleportEntity(client, g_vec_Team2_send_pos, NULL_VECTOR, NULL_VECTOR);
			SDKUnhook(client, SDKHook_SpawnPost, PlayerMissedSendOutTeam2);
		}
	}
}
			...
			if( client_info[i][VOTEINFO_CLIENT_ITEM] == item_info[vote_yes_index][VOTEINFO_ITEM_INDEX] )
			{
				if( IsValidClient(client_info[i][VOTEINFO_CLIENT_INDEX]) )
				{
					...
					if( !IsInDuel(client_info[i][VOTEINFO_CLIENT_INDEX]) )
					{
						...
						StoreClientNameAndColor(client_info[i][VOTEINFO_CLIENT_INDEX]);
					}
				}
...
DisplayCaptain1Vote()
{
	...
	SetMenuExitButton(menu, false);
	VoteMenu(menu, cl_arr, num_participants, 20);
		...
		if( (num_items > 1) && (item_info[0][VOTEINFO_ITEM_VOTES] == item_info[1][VOTEINFO_ITEM_VOTES]) )
		{
			...
			if( IsValidClient(cl_index_2) )
			{
				...
				if( BerimbauGetRank(cl_index_1) < BerimbauGetRank(cl_index_2) )
				{
					...
					ModifyCaptainNameAndColor(cl_index_1);
				}
				else
				{
					SetCaptain_1(cl_index_2);
					ModifyCaptainNameAndColor(cl_index_2);
				}
			}
			else
			{
			...
			else
			{
				...
				else
				{
					...
					if( IsCaptain_1(param1) )
					{
						...
						ModifyClientNameAndColor(cl_index);
						...
						else if( CountNumParticipants_In_Arena(true, cl_index) == 1 )
						{
							...
							for( new i = 1; i <= MaxClients; i ++ )
							{
								if( (g_Client_Data[i][ARENA_VALUE] == 1) && (g_Client_Data[i][TEAM_VALUE] == 1) )
								{
									...
									if( IsPlayerAlive(cl_index) )
									{
										DispatchKeyValueVector(cl_index, "origin", g_vec_Team1_send_pos);
									}
									else
									{
										SDKHook(cl_index, SDKHook_SpawnPost, PlayerMissedSendOutTeam1);
									}
									DisplayCaptainOptionsMenu(g_Captain_1, 1);
									break;
								}
						...
						else if( CountNumParticipants_In_Arena(true, cl_index) == 1 )
						{
							...
							for( new i = 1; i <= MaxClients; i ++ )
							{
								if( (g_Client_Data[i][ARENA_VALUE] == 1) && (g_Client_Data[i][TEAM_VALUE] == 2) )
								{
									...
									if( IsPlayerAlive(cl_index) )
									{
										DispatchKeyValueVector(cl_index, "origin", g_vec_Team2_send_pos);
									}
									else
									{
										SDKHook(cl_index, SDKHook_SpawnPost, PlayerMissedSendOutTeam2);
									}
									DisplayCaptainOptionsMenu(g_Captain_2, 2);
									break;
								}
...
{
	RestoreClientNameAndColor(client);
...
stock StoreClientNameAndColor(client)
{
	GetClientInfo(client, "vs_color", g_Client_Color[client], SIZE_COLOR);
	GetClientName( client, g_Client_Name[client], MAX_NAME_LENGTH);
}

stock ModifyCaptainNameAndColor(client) 
{
	static String:new_name[MAX_NAME_LENGTH];

	if( client == g_Captain_1 )
	{
		Format(new_name, sizeof(new_name), "[Captain 1]%s", g_Client_Name[client]);
		SetClientInfo(client, "name", new_name);
		ClientCommand(client, "vs_color 1.000000 1.000000 0.000000");
	}
	else if( client == g_Captain_2 )
	{
		Format(new_name, sizeof(new_name), "[Captain 2]%s", g_Client_Name[client]);
		SetClientInfo(client, "name", new_name);
		ClientCommand(client, "vs_color 1.000000 0.000000 0.000000");
	}
}

stock ModifyClientNameAndColor(client)
{
	static String:new_name_team[MAX_NAME_LENGTH];

	if( g_Client_Data[client][TEAM_VALUE] == 1 )
	{
		Format(new_name_team, sizeof(new_name_team), "[Team 1]%s", g_Client_Name[client]);
		SetClientInfo(client, "name", new_name_team);
		ClientCommand(client, "vs_color 1.000000 1.000000 0.000000");
	}
	else if(  g_Client_Data[client][TEAM_VALUE] == 2 )
	{
		Format(new_name_team, sizeof(new_name_team), "[Team 2]%s", g_Client_Name[client]);
		SetClientInfo(client, "name", new_name_team);
		ClientCommand(client, "vs_color 1.000000 0.000000 0.000000");
	}
}

stock RestoreClientNameAndColor(client)
{
	SetClientInfo(client, "name", g_Client_Name[client]);
	ClientCommand(client, "vs_color %s", g_Client_Color[client]);
}
